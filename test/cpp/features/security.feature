    ...
    # @sbdl-begin
      req-sec-0001 is requirement {
        ...
        "Including provenance gives confidence that the container images haven't been tampered with
         and can be securely traced back to its source code. The primary purpose is to enable
         [verification](https://slsa.dev/spec/v1.0/verifying-artifacts) that the container image was
         built as expected. Consumers have some way of knowing what the expected provenance should
         look like for a given container image and then compare each container image's actual
         provenance to those expectations. Doing so prevents several classes of [supply chain
         threats](https://slsa.dev/spec/v1.0/threats)."
      }
    # @sbdl-end
    ...
    # @sbdl-begin
      req-sec-0003 is requirement {
        ...
        "Providing a Software Bill of Materials (SBOM) enables consumers to identify and manage
         security risks associated with the software components included in the container images. It
         helps identify known vulnerabilities, license compliance issues, and potential supply
         chain risks."
      }
    # @sbdl-end
