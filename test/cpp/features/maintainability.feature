Feature: Maintainability
  ...
  Rule: Automatic updates
    embedded-devcontainer *SHOULD* provide support for automatic updates when consumed as a dependency.

    Providing support for automatic updates when embedded-devcontainer is consumed as a dependency helps ensure that users always have access to the latest features, bug fixes, and security patches.
  ...
  Rule: Container image size
    embedded-devcontainer *SHOULD* aim to keep its container image size as small as possible without compromising functionality.
    ...
    By optimizing the container image size, embedded-devcontainer can provide a more efficient and user-friendly experience for developers.
